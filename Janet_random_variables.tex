...
\textbf{TA definitions}:
\begin{itemize}
	\item \textit{E didn't like this} A random set s c[0,T] is distributed according to PP($\lambda$) if s = $\{t_1, t_2, \dots , t_n\}$ and n is Po($\lambda T$).  Longer time and higher intensity lead to more events.  There are no other requirements.  ????
	\item \textit{There was another but it used vocabulary/concepts we haven't done}
\end{itemize}

TA notes: 
Note that the busses coming at random times are modeled as Poisson processes.  
Can take any bus therefore some aggregation of these. 
Sums of independent Poisson are Poisson.  Unions of Poisson are Poisson.  {\tiny (TA 11/30 lecture)}
...
Like binomial with $\lambda = np$ and $n \to \infty$ {\tiny (TA 11/30 lecture)} \hfill \\
...
{\tiny (11/25 TA Lecture)} 
If $X \sim Po(\lambda_X)$ and  $Y \sim Po(\lambda_Y)$ and X and Y are independent then $X + Y \sim Po(\lambda_X + \lambda_Y)$.  {\tiny Written again on 12/2 as:}  $X \sim Po(\mu)$, $Y \sim Po(\nu)$, $X+Y \sim Po(\mu + /nu)$ 
...
TA mumble on 12/7: If someone gives us info about a um of 2 R.V.s and asks us to say something about 1 R.V. then we use Binomial, not Poisson.  \hfill \\
...
\end{align*} 
{\tiny Note: this binomial dist was also derived on 12/2}  \hfill \\
Addition: $\sum_x P_X(x \mid W=w) = \sum_x \frac{P(X=x, W=w)}{P(W=w)} = \frac{P(W=w)}{p(W=w)} = 1$  \hfill \\
 \hfill \\
...
\begin{align*}
	F_T(s) &= P(T \leq s | \mbox{one event in} (0,t) \mbox{, process rate } \lambda) \\
		...
		&= s/t
\end{align*}
To get the density, differentiate.  $f_T(s) = 1/$ for $0 \leq s \leq t$ and 0 otherwise.   {\tiny This was re-derived on 12/2.}
...
\subsection{Min of 2 Exponentials}
{\tiny (11/30 TA Lec)} If $Y_1 \sim expo(2/hr)$ and $Y_2 \sim expo(4/hr)$ and $Y_1$ and $Y_2$ are independent then min($Y_1,Y_2) \sim expo(6/hr)$ 

\subsection{$P(Y_{min} > y)$}
{\tiny (11/30 TA Lec)}  
\begin{align*}
	P(Y_{min} > y) &= P(Y_1 > y \mbox{ and } Y_2 < y) \\
		&= P(Y_1 > y)P(Y_2 > Y)  \\
		&= exp(-\frac{4}{\mbox{hr}} y) exp(-\frac{2}{\mbox{hr}} y) \\
		&= exp(-\frac{6}{\mbox{hr}} y)
\end{align*}
...
\subsection{Adding exponentials and knowing the total}
{\tiny (12/4 Lecture)}  $X \sim expo(\lambda)$, $Y \sim expo(\lambda)$.  
W = X + Y = time for both of them to happen.  
Note that X and W are not independent.
Would be gamma with param 2 but just think of them as sum of 2 exponentials.    \hfill \\
Previously derived: $(X | W=w)$ is $U[0,w]$.  (PDF = constant 1/w).  \hfill \\
 So $P(X \leq x \mid W = w) = x/w$ for $0 \leq x \leq w$.  (The cdf) \hfill \\
 \hfill \\
 
 Now let $V = \frac{X}{X+Y} = \frac{X}{W}$ and consider V and W. \hfill \\
 \begin{align*}
 	P(V \leq v \mid W=w) &= P(X/W \leq v \mid W=w) \\
		&= P(X \leq vw \mid W=w) \\
		&= \frac{vw}{w} \\
		&= v \mbox{ for } 0 \leq vw \leq w \\
	\mbox{ differentiating:} & \\
	f_V(v \mid W=w) &= 1
 \end{align*}
 \textbf{W, V are independent}, because neither function nor range depends on w.  \hfill \\
 
 Do again using conditional probability. 
 {\tiny (12/7 TA lecture)} $X, Y \sim expo(2)$.  Define $W \equiv X + Y$ and look at $f_X \mid W(x,w)$. 
 \begin{align*}
 	f_{X|W}(x,w) & \equiv \frac{f_{X,W}(x,w)}{f_W(w)} \\
		& f_W(w): \mbox{ sum of 2 exponentials is gamma.} \\
		&=  \frac{f_{X,Y}(x, w-x)}{4w \cdot \mbox{exp}(-2w)} \\
		&=  \frac{2\cdot \mbox{exp}(-2w) 2\cdot \mbox{exp}(-2w)}{4w \cdot \mbox{exp}(-2w)} \\
		&=  \frac{1}{w} \mbox{ for } x < w, 0 < x  \\
		& \mbox{ still have to pay attention to range.}
 \end{align*}
 $X \mid W$ is U$[0,W]$.  \hfill \\
 It is a valid pdf for x since $\int_0^w 1/w dx = w/w = 1$

\section{Conditional Statements \& Random Variables}
Continuous: $f_{X \mid Y}(x \mid y) = \frac{f_{X,Y}(x,y)}{f_Y(y)}$ \hfill \\
Note: $f_{X \mid Y}(x \mid y) = f_{X \mid Y}(x) = f_X(x \mid Y=y)$

If X doesn't depend on Y: \textbf{(independence)} \hfill \\
\textit{discrete}:  \hfill \\
$P_X(x \mid Y=y) = \frac{p_{X,Y}(x,y)}{p_Y(y)} = \frac{p_X(x) p_Y(y)}{p_Y(y)}  = p_X(x)$  \hfill \\
\textit{continuous}:  \hfill \\
$f_X(x \mid Y=y) = \frac{f_{X,Y}(x,y)}{f_Y(y)} = \frac{f_X(x) f_Y(y)}{f_Y(y)} = f_X(x)$  \hfill \\
If we find $f_{X \mid Y}(x \mid y)$ does not depend on y, they X and Y are independent.
%If $y = g_1(x)$ ($f_X(x \mid Y=y)$ doesn't depend on y):  \hfill \\

\section{Conditional Expectation}
Elizabeth: $E(Y) = E(E(Y \mid X))$. 
Erick: $E(Y) = E(E(Y \mid X)) = E_X(E_Y(Y \mid X))$.  $Y \mid X$ is described by $f_{Y \mid X}$ Integrate Y out first. 
